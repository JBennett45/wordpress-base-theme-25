class CustomAccordionElement extends HTMLElement {
	...
	connectedCallback() {
		if (this.accordion_instance) {
			this.accordion_instance
				.querySelectorAll('.jb-custom-accordion-cst__item__title')
				.forEach((accordion) => {
					accordion.addEventListener('click', (e) => {
						let parent_item = accordion.closest(
							'.jb-custom-accordion-cst__item',
						);

						document
							.querySelectorAll('.jb-custom-accordion-cst__item')
...
customElements.define('custom-accordion', CustomAccordionElement);
