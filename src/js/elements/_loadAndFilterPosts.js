import { filteringLoadState } from './../functions/_loader';
class PostTypeLoadAndFilter extends HTMLElement {
  constructor() {
    ...
    this.post_wrapper = this.querySelector('.custom-posts-main-wrap');
    this.btn_load_more = this.querySelector('.load-more-posts-btn-cst');
    this.filter_instance = this.querySelector('.custom-tax-filter-wrapper');
  ...
  connectedCallback() {
    ...
    if (this.filter_instance && this.filter_taxonomy) {
      this.filter_instance
        .querySelectorAll('.custom-tax-filter-item')
        .forEach((filter) => {
  ...
  loadMorePosts = async (e) => {
    ...
    try {
      ...
      if (res.success === 1) {
        ...
        console.error(
          'Something went wrong during the loadmore request, please contact admin.',
        );
      }
    } catch (err) {
  ...
  filterPosts = async (e) => {
    ...
    try {
      ...
      if (res.success === 1) {
        ...
        console.error(
          'Something went wrong during the filter request, please contact admin.',
        );
        filteringLoadState(this.post_wrapper, 'end');
      }
    } catch (err) {
